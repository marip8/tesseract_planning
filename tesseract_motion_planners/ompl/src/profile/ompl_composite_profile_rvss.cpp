...
{
  for (const InstructionPoly& inst : instruction)
  {
    const auto& goal_instruction = inst.as<MoveInstructionPoly>();
    if (goal_instruction.getMoveType() != MoveInstructionType::FREESPACE)
